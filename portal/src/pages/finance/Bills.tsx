            ...
            <tbody>
              {bills.map(bill => (
                <tr key={bill.id} style={{ borderTop: '1px solid #374151' }}>
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    {bill.status === 'pending' && (
                      <button onClick={() => handleApprove(bill.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: '#3b82f622', color: '#3b82f6', border: 'none', borderRadius: 4, padding: '3px 10px', cursor: 'pointer', fontSize: 12 }}>
