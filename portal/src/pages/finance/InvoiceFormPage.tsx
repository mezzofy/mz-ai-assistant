...
export default function InvoiceFormPage() {
  ...
  function addLineItem() {
    setForm((f) => ({ ...f, line_items: [...f.line_items, { description: '', quantity: 1, unit_price: 0, tax_rate: 0 }] }))
  }
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={() => navigate('/mission-control/finance/invoices')}
      ...
      <div className="rounded-xl border p-6 space-y-6" style={{ background: '#111827', borderColor: '#1E2A3A' }}>
        ...
        <div className="border-t pt-5" style={{ borderColor: '#1E2A3A' }}>
          ...
          {/* Column headers */}
          <div className="flex gap-2 mb-2 text-xs" style={{ color: '#6B7280' }}>
            ...
            {form.line_items.map((item, i) => (
              <div key={i} className="flex gap-2 items-center">
                ...
                <input
                  ...
                  type="number"
                  value={item.tax_rate}
                  onChange={(e) => updateLineItem(i, 'tax_rate', parseFloat(e.target.value) || 0)}
                  className="px-3 py-2 rounded-lg text-sm text-white border outline-none"
                  style={{ flex: 1, background: '#1E2A3A', borderColor: '#374151' }}
                />
                ...
                {form.line_items.length > 1 && (
                  <button
                    ...
                    style={{ background: 'rgba(220,38,38,0.15)', color: '#dc2626', width: 32 }}
                  >
                    ✕
                  </button>
                )}
                {form.line_items.length === 1 && <div style={{ width: 32 }} />}
              </div>
            ))}
          </div>
      ...
      <div className="flex justify-end gap-3 pb-6">
        ...
        <button
          ...
          disabled={saving || !form.customer_id || !form.due_date}
          className="px-5 py-2 rounded-lg text-sm font-medium text-white transition-all"
          style={{
            background: '#f97316',
            opacity: saving || !form.customer_id || !form.due_date ? 0.6 : 1,
          }}
        >
