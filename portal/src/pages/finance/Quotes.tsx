  ...
  return (
    <div style={{ padding: 24, color: '#F9FAFB' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>Quotes</h1>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <button onClick={() => navigate('/mission-control/finance/quotes/new')}
            ...
            <tbody>
              {quotes.map(q => (
                <tr key={q.id} style={{ borderTop: '1px solid #374151' }}>
                  ...
                  <td style={{ padding: '10px 14px' }}>
                    {q.status === 'draft' && (
                      <button style={{ background: '#3b82f622', color: '#3b82f6', border: 'none', borderRadius: 4, padding: '3px 10px', cursor: 'pointer', fontSize: 12, marginRight: 6 }}>
                        Send
                      </button>
                    )}
                    {q.status === 'accepted' && (
                      <button style={{ background: '#16a34a22', color: '#16a34a', border: 'none', borderRadius: 4, padding: '3px 10px', cursor: 'pointer', fontSize: 12 }}>
                        → Invoice
                      </button>
                    )}
                  </td>
