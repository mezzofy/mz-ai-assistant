...
- **MSAL blocking pattern (WARNING):** `msal.ConfidentialClientApplication` methods are synchronous. Called from `async def` handlers in `ms_oauth.py` and `ms_token_service.py` — blocks event loop per call. Acceptable for low-traffic EC2; fix in v1.1 with `run_in_executor`.
