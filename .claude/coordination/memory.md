...
- **mz-ai-assistant mobile v1.3.0 (2026-03-05, in progress):** Files tab — pull-to-refresh, download with progress (react-native-fs), in-app viewer (image/video/markdown/text). New packages: react-native-fs, react-native-video, react-native-markdown-display. Requires native rebuild after npm install. Review: PASS. Plan: `plans/llm-usage-stats-plan.md` — Backend Task 1 (GET /llm/usage-stats) then Mobile Task 2 (wire AIUsageStatsScreen).
