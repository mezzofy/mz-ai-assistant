...
- **Push notifications v1.28.0 (2026-03-16, Lead review: PASS):** `user_devices` table (FCM token registry, UNIQUE on token for multi-user device support) + `users.push_notifications_enabled` column. `get_user_push_targets(user_id)` helper in push_ops.py — DB lookup respects pref toggle, returns [] on any failure (non-fatal). Push fires in `_run_agent_task` (guard: `user_id != "system"`) and `_run_chat_task` (both success + failure paths). Mobile: RN Firebase 20.x, `initPushNotifications()` in App.tsx `useEffect([isLoggedIn, notifications])`, `unregisterPushDevice()` called in logout BEFORE `clearTokens()`. Dynamic import in authStore avoids circular dep. Backend ready to deploy; Mobile needs version bump to v1.28.0 + npm install + APK build.
