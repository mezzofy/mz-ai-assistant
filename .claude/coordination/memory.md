...
- **HRAgent added 2026-03-06:** 6th department agent (`hr`). Webhooks: `employee_onboarded`, `employee_offboarded`, `leave_request_submitted` → `"hr"`. Scheduler: `weekly_hr_summary` (Fri 5PM SGT), `monthly_headcount` (1st of month 9AM SGT). 28 tests passing. Companion config changes pending (Backend Agent): `config/roles.yaml` (hr_viewer/hr_manager), `app/core/rbac.py` (VALID_ROLES/VALID_DEPARTMENTS), `tasks/beat_schedule.py` (2 new jobs), `config/config.example.yaml` (#hr channel + notifications).
