...
- **BUG-015 (2026-03-12) — agent_task UUID type mismatch in context.processor:** `context/processor.py` passes a Python `UUID` object where PostgreSQL asyncpg expects a `str`. Error: `"invalid input for query argument $4: UUID('...') (expected str, got UUID)"`. Non-fatal (just a logging failure) but should be fixed by casting: `str(session_id)` in the SQL parameter dict.
