...
class LLMManager:
    ...
    def _load_brand_guidelines_text(self) -> str:
        """
        Load brand guidelines from knowledge/brand/guidelines.md (or .json).
        Returns the file content as a string, or a compact hardcoded fallback
        if the file is missing — so this method never raises.
        """
        server_root = Path(__file__).parent.parent.parent
        candidates = [
            server_root / "knowledge" / "brand" / "guidelines.md",
            server_root / "knowledge" / "brand" / "guidelines.json",
            server_root / "knowledge" / "brand_guidelines.md",
        ]
        for path in candidates:
            if path.exists():
                try:
                    content = path.read_text(encoding="utf-8")
                    # Append logo availability note for AI-generated documents
                    logo_dir = path.parent
                    logo_note_parts = []
                    ...
                    logger.warning(f"Failed to read brand guidelines from {path}: {e}")

        # Hardcoded fallback — mirrors knowledge_ops.py defaults
        return (
            "# Mezzofy Brand Guidelines (defaults)\n"
            "## Colors\n"
            ...
            "## Document Standards\n"
            "- Footer: 'Generated by Mezzofy AI Assistant'\n"
            "- Primary headings: orange (#f97316)\n"
            "- Body text: black; backgrounds: white\n"
        )
    ...
    async def generate_document_with_skill(
        ...
        # Build full prompt with brand guidelines + context
        brand_block = self._load_brand_guidelines_text()
        user_content = (
            f"BRAND GUIDELINES (apply strictly to this document):\n"
            f"{brand_block}\n\n"
            f"---\n\n"
            f"{prompt}"
        )
